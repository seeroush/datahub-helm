...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| datahub-gms.sql.datasource.username.secretRef | string | `"mysql-secrets"` | Secret that contains the GMS SQL password (overrides global value) |
...
| datahub-gms.sql.datasource.username.value | string | `"mysql-username"` | Alternative to using the secret above, uses raw string value for GMS SQL login (overrides global value) |
